...
output.write(f"Total: ${str(total_net)}\n")
output.write(f"Average Change: ${str(round(average_change,2))}\n")
output.write(f"Greatest Increase in Profits: {greatest_date} (${str(greatest_inc)})\n")
output.write(f"Greatest Decrease in Profits: {worst_date} (${str(greatest_dec)})\n")
